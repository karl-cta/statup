// Maintenance window: a day picked on a calendar, a time typed freely, a
// duration in hours and minutes, and the end they lead to. Times are the
// instance's wall clock, held in Date objects read through their UTC fields
// so the browser's own zone never shifts them.
(function () {
    "use strict";

    const root = document.querySelector("[data-schedule]");
    const copyNode = document.getElementById("schedule-copy");
    if (!root || !copyNode) return;
    ...
    const durationMinutes = () => digits(hours) * 60 + digits(minutes);
    ...
    function restore() {
        ...
        const end = parseWall(endValue.value);
    ...
    function header() {
        ...
        prev.disabled = shownMonth.getTime() <= monthOf(firstAllowed()).getTime();
        bar.append(prev, title, navButton(copy.next_month, "m9 18 6-6-6-6", 1));
        return bar;
    }
    ...
    function dayButton(date) {
        ...
        button.disabled = date.getTime() < firstAllowed().getTime();
        return button;
    }
    ...
    function footer() {
        ...
        reset.textContent = copy.start_right_away;
    ...
    function showMonth(month, focus) {
        ...
        const allowed = inMonth.getTime() < firstAllowed().getTime() ? firstAllowed() : inMonth;
        focusDay(allowed);
    }
    ...
    function moveFocus(event, current) {
        ...
        if (!target) return;
        event.preventDefault();
        if (target.getTime() < firstAllowed().getTime()) target = firstAllowed();
