//! Icon routes - upload, library browsing, picker, delete.
...
#[derive(Template)]
#[template(path = "icons/gallery.html")]
struct IconGalleryTemplate {
    csrf_token: String,
    icons: Vec<Icon>,
    i18n: I18n,
}
...
pub async fn upload(
        ...
        .map_err(|e| AppError::Validation(format!("{}: {e}", i18n.t("validation.generic_error"))))?
    {
        ...
        match name.as_str() {
            "csrf_token" => {
                // CSRF is validated by middleware, just consume the field
